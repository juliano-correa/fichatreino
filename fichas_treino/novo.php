<?php
// Fichas de Treino - Nova com Grupos de Exercícios
$titulo_pagina = 'Nova Ficha';
$subtitulo_pagina = 'Criar Ficha de Treino';
...
// Obter lista de exercícios da tabela exercises (se existir)
$tem_exercicios_banco = false;
...
try {
    ...
    if (!empty($exercicios_banco)) {
        $tem_exercicios_banco = true;
    ...
    if (empty($aluno_id)) {
        ...
        foreach ($grupos_post as $grupo) {
            ...
            foreach ($exercicios as $ex) {
                // Aceita tanto exercise_id (do select) quanto nome digitado
        ...
        if (!$tem_exercicio_valido) {
            ...
            try {
                ...
                $sql = "INSERT INTO workouts (
                            gym_id, aluno_id, nome, descricao, exercicios, frequencia, objetivo, ativa
                        ) VALUES (
                            :gym_id, :aluno_id, :nome, :descricao, :exercicios, :frequencia, :objetivo, :ativa
                        )";
...
<form method="POST" id="formFicha">
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white py-3">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="bi bi-clipboard-check me-2"></i>Dados da Ficha</h5>
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="objetivo" name="objetivo">
                        ...
                        <option value="Perda de Peso" <?= ($_POST['objetivo'] ?? '') == 'Perda de Peso' ? 'selected' : '' ?>>Perda de Peso</option>
                        <option value="Condicionamento" <?= ($_POST['objetivo'] ?? '') == 'Condicionamento' ? 'selected' : '' ?>>Condicionamento</option>
                        <option value="Força" <?= ($_POST['objetivo'] ?? '') == 'Força' ? 'selected' : '' ?>>Força</option>
                        <option value="Resistência" <?= ($_POST['objetivo'] ?? '') == 'Resistência' ? 'selected' : '' ?>>Resistência</option>
                        <option value="Definição" <?= ($_POST['objetivo'] ?? '') == 'Definição' ? 'selected' : '' ?>>Definição</option>
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="frequencia" name="frequencia">
                        <option value="">Selecione...</option>
                        <option value="1x semana" <?= ($_POST['frequencia'] ?? '') == '1x semana' ? 'selected' : '' ?>>1x semana</option>
                        <option value="2x semana" <?= ($_POST['frequencia'] ?? '') == '2x semana' ? 'selected' : '' ?>>2x semana</option>
                        <option value="3x semana" <?= ($_POST['frequencia'] ?? '') == '3x semana' ? 'selected' : '' ?>>3x semana</option>
                        <option value="4x semana" <?= ($_POST['frequencia'] ?? '') == '4x semana' ? 'selected' : '' ?>>4x semana</option>
                        <option value="5x semana" <?= ($_POST['frequencia'] ?? '') == '5x semana' ? 'selected' : '' ?>>5x semana</option>
                        <option value="6x semana" <?= ($_POST['frequencia'] ?? '') == '6x semana' ? 'selected' : '' ?>>6x semana</option>
                        <option value="Diária" <?= ($_POST['frequencia'] ?? '') == 'Diária' ? 'selected' : '' ?>>Diária</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <label for="ativa" class="form-label">Status</label>
                    <div class="form-check mt-2">
                        <input class="form-check-input" type="checkbox" id="ativa" name="ativa" value="1" <?= !isset($_POST['ativa']) || $_POST['ativa'] ? 'checked' : '' ?>>
                ...
                <div class="col-md-12">
                    ...
                    <textarea class="form-control" id="descricao" name="descricao" rows="2" 
                              placeholder="Instruções gerais, intervalo entre séries, dicas..."><?= $_POST['descricao'] ?? '' ?></textarea>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body p-0">
            ...
            <div class="text-center py-5 text-muted" id="semGrupos">
    ...
    <div class="d-flex justify-content-end gap-2">
        <a href="<?= base_url('fichas_treino/index.php') ?>" class="btn btn-outline-secondary">
            <i class="bi bi-x-lg me-1"></i>Cancelar
        </a>
        <button type="submit" class="btn btn-primary">
            <i class="bi bi-check-lg me-1"></i>Salvar Ficha Completa
        </button>
    </div>
</form>
...
<!-- Template de linha de exercício - COM select do banco -->
...
function adicionarExercicioPreenchido(grupoDiv, dadosExercicio, grupoIndex) {
    ...
    if (dadosExercicio && dadosExercicio.nome) {
...
// Adicionar um grupo inicial por padrão se não houver POST
